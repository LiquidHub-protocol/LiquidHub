...
| **RangeManager.sol** | Executes the DEX position lifecycle after validating a fresh engine decision. Rebalances use one atomic bounded swap when possible and a resumable, freshly recomputed progressive path for larger TVL. |
...
| **SecureBotModule.sol** | Gnosis Safe module restricting relayed bot selectors and storing the permissionless progressive-rebalance state. Each continuation is revalidated by RangeManager and shares a cumulative oracle-USD swap budget capped by the initial RangeManager NAV. The Safe can cancel a stuck cycle without moving principal. |
