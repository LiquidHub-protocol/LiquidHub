...
1. **Liquidity provision**: WETH and USDC are deployed into a Uniswap V3 concentrated liquidity position, earning trading fees. A per-pool `RangeStrategyEngine` combines an analytical anchor, fixed multi-scenario evaluation, bounded online adaptation and false-start protection to publish an exact on-chain action and target range. A shallow spot-only range exit requires tactical-TWAP, elapsed-epoch or material-depth confirmation; deep and persistent range liveness remains independent. Every new asymmetric target must preserve the governed skew budget around the live execution tick, preventing a wide total range from hiding a price placed only a few ticks from one boundary.
...
All contracts from the Exposed pool are included, plus the Delta Neutral hedge components:
...
| **DnDepositLib.sol** | Stateless linked library coordinating the atomic Delta Neutral deposit plan, AAVE collateral/debt opening and post-deposit hedge checks. It holds no funds, owner or mutable state. |
...
| **RangeStrategyEngine.sol** | Per-pool Delta Neutral decision contract combining an analytical controller, fixed multi-scenario optimizer, bounded online adaptation and false-start protection. Holds no funds and cannot move liquidity or alter the hedge; publishes the versioned action, reason code, exact target ticks, validity and decision hash revalidated before execution. It adds hedge drift, borrow conditions and current/stressed health-factor constraints, while critical hedge and health-factor safety paths remain independent from shallow range-exit confirmation. |
...
| **RangeManager.sol** | Executes the DEX position lifecycle after validating a fresh engine decision. Rebalances use one atomic bounded swap when possible or a resumable, freshly recomputed progressive path for larger TVL, with post-mint hedge synchronization for `RANGE_AND_HEDGE`. |
| **RangeOperations.sol** | Library for tick alignment, liquidity calculations, bounded swaps, valuation and fee accounting. |
| **PauseController.sol** | Per-pool bounded circuit breaker for inflows, withdrawals and the deposit cooldown. Holds no funds and leaves documented strategy-maintenance, hedge-safety and emergency-recovery paths available. |
| **SecureBotModule.sol** | Gnosis Safe module restricting relayed bot selectors and storing the permissionless progressive-rebalance state. Each continuation is revalidated by RangeManager and shares a cumulative oracle-USD swap budget capped by the initial RangeManager NAV. DN begin/chunk/finalize steps also defer to permissionless HF repair below the live on-chain trigger. The Safe can cancel a stuck cycle without moving principal. |
