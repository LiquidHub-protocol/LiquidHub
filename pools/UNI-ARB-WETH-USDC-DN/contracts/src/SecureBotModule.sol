...
contract SecureBotModule {
    ...
    constructor(
        ...
        require(
            _safe != address(0) && _botAddress != address(0) && _rangeManager != address(0)
                && _vault != address(0) && _pauseController != address(0) && _hedgeManager != address(0) && _treasury != address(0),
            "E_ZERO"
        );
        ...
        // AUDIT H-01 : signature à 2 args (collateral USDC exact + borrow WETH). Ancien sélecteur
        // 0x39ed9306 (supplyAndBorrow(uint256)) RETIRÉ — le contrat n'expose plus que la version 2-args.
        allowedFunctions[0x8fc9f880] = true; // supplyAndBorrow(uint256,uint256)
        ...
        withinDailyLimit 
    {
        ...
        if (selector == MINT_INITIAL_SELECTOR) _requirePostMintHedgeOk();
    ...
    function _requirePostMintHedgeOk() private view {
    ...
    function _isCoreSelector(bytes4 selector) private pure returns (bool) {
        return
            ...
            selector == 0x8fc9f880 || // supplyAndBorrow(uint256,uint256)
